class Solution {
    public boolean canPartition(int[] nums) {
        ...
        /* Create a boolean array to store the subset sums */
        ...
        for (int num : nums) {
            ...
            /* Traverse the subsetSums array in reverse 
            to avoid using an element multiple times */
            for (int j = targetSum; j >= num; j--) {
                
                /* If j-num is achievable, set j as achievable */
                subsetSums[j] = subsetSums[j] || subsetSums[j - num];
            }
        }
      
        /* The result is whether the targetSum is as a subset sum */
